...
public abstract class PiezaAjedrez extends Pieza {
	...
	public final boolean movimientoDentroLimites (int toFila, int toColumna) {
		return toFila>=0 && toFila<=7 
				&& toColumna>=0 && toColumna<=7
				&& (getFila()!=toFila || getColumna()!=toColumna);
	}
	...
	public final boolean puedeMover (int toFila, char toColumna){
		return movimientoDentroLimites(8-toFila, toColumna-'a');
	}
	 ...
	 * @return retorna la columna en la que se encuentra la pieza en un tablero de ajedrez
	 */
	...
	protected void casillasVisitables (IList<Casilla> resultado,int incFila, int incColumna){
		...
		PiezaAjedrez pieza = queHay(i, j);
		boolean hayPiezaEnemiga = false;
		boolean hayPiezaAmiga = pieza!=null && 
				pieza.getJugador().getId() == getJugador().getId();
		
		while(movimientoDentroLimites(i, j) && !hayPiezaEnemiga 
				&& !hayPiezaAmiga) {
			pieza = queHay(i, j);
			if(pieza!=null) {
				hayPiezaEnemiga = pieza.getJugador().getId() != getJugador().getId();
				hayPiezaAmiga = !hayPiezaEnemiga;
			}
			
			if(hayPiezaEnemiga || pieza == null) {
	 ...
	 * Una casilla es visitable si y sólo si está vacía u ocupada por una pieza contraria.
