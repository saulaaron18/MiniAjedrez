...
public class TestIntegridadGestorHistorial {
	...
	private void testMetodo (Metodo metodo) {
		...
		try {
			...
			assertEquals("Se ha encontrado el metodo " + metodo.getNombre() + " pero no coincide el tipo devuelto esperado (" +
			metodo.getTipoDevuelto()+")" + method.getReturnType().getName(), metodo.getTipoDevueltoConvertido(), method.getReturnType().getName()) ;
