...
public class TestGestorHistorial {
	...
	public TestRule  globalTimeout = new DisableOnDebug(Timeout.millis(200)); // 100 milisegundos máximos por test
	...
	@Test
	public void testVaciadoRehacer() throws EmptyStackException {
		GestorHistorialTrace gestor = new GestorHistorialTrace (); 
		...
		gestor.guardarMovimiento(movs[2]);

		Stack<Movimiento> pilaRehacer = gestor.getPilaRehacer();
		Stack<Movimiento> pilaDeshacer = gestor.getPilaDeshacer();
		assertTrue(gestor + "La pila de rehacer debe estar vacía después de guardar un movimiento. " , size(pilaRehacer) == 0);
		//2018-11-03 ALGM: Se añade para verificar que no se modifica indevidamente la pila de deshacer
		assertTrue(gestor + "La pila de deshacer debe albergar un movimineto " , size(pilaDeshacer) == 1);
		try {
			gestor.rehacer();
			fail (gestor + "Debería haber lanzado una excepción al intentar rehacer con la pila vacía. " ); 
		} catch (MovimientoException e) {
			// correcto!
			Logger.getGlobal().info("Se ha generado corectamente la excepción MovimientoException cunado correspondía al intentar rehacer un movimiento sin que haya movimientos para rehacer");
		} 
	}
	...
	public void testExcepciones() {
		...
		try {
			...
			// correcto!
			Logger.getGlobal().info("Se ha generado corectamente la excepción MovimientoException cunado correspondía al intentar deshacer un movimiento sin que haya movimientos para deshacer");
		} catch (Exception e) {
